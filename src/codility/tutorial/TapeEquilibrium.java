 ...
 * 
 *         A[0] = 3 A[1] = 1 A[2] = 2 A[3] = 4 A[4] = 3 We can split this tape
 *         in four places:
 * 
 ...
 *         For example, given:
 * 
 *         A[0] = 3 A[1] = 1 A[2] = 2 A[3] = 4 A[4] = 3 the function should
 *         return 1, as explained above.
 * 
 *         Write an efficient algorithm for the following assumptions:
 * 
 *         N is an integer within the range [2..100,000]; each element of array
 *         A is an integer within the range [−1,000..1,000]. Copyright 2009–2020
 *         by Codility Limited. All Rights Reserved. Unauthorized copying,
...
public class TapeEquilibrium {
	...
	public int solution(int[] A) {
		...
		long minDiff = Long.MAX_VALUE, currentSum = 0;

		for (int i = 0; i < A.length - 1; i++) {
			currentSum += A[i];
			if (minDiff > Math.abs(currentSum - (sum - currentSum))) {
				minDiff = (int) Math.abs(currentSum - (sum - currentSum));
				if (minDiff == 0) {
					return 0;
				}
			}
