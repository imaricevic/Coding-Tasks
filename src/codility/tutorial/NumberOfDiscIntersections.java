 ...
 *         An array A of N non-negative integers, specifying the radiuses of the
 ...
 *         10,000,000.
 * 
 *         Given array A shown above, the function should return 11, as
 *         explained above.
 * 
...
public class NumberOfDiscIntersections {
	...
	public int solution(int[] A) {
		...
		for(int i = 0; i < A.length; i++) {
			open++;
		    while (endIndex < A.length - 1 && (A.length - 1 == i || end[endIndex] < start[i + 1])) {
		        count += --open;
		        if(count > 10_000_000) {
		        	return -1;
		        }
