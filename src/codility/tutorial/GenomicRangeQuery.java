 ...
 * 
 *         P[0] = 2 Q[0] = 4 P[1] = 5 Q[1] = 5 P[2] = 0 Q[2] = 6 The answers to
 *         these M = 3 queries are as follows:
 * 
 ...
 *         between positions 0 and 6 (the whole string) contains all
 *         nucleotides, in particular nucleotide A whose impact factor is 1, so
 *         the answer is 1. Write a function:
 * 
 ...
 * 
 *         P[0] = 2 Q[0] = 4 P[1] = 5 Q[1] = 5 P[2] = 0 Q[2] = 6 the function
 *         should return the values [2, 4, 1], as explained above.
 * 
 ...
 *         consists only of upper-case English letters A, C, G, T.
 * 
...
public class GenomicRangeQuery {
	...
	public int[] solution(String S, int[] P, int[] Q) {
        ...
        if(S == null || P == null || Q == null || P.length != Q.length) {
			return null;
		}
		
		int[][] sum = new int[3][S.length()];
		int sumA = 0, sumC = 0, sumG = 0;
 		...
 		for(int i=0; i < P.length; i++) {
 			...
 			result[i] = calculate(P[i], Q[i], sum);
 		}
		
		return result;
    }
	
	private int calculate(int begin, int end, int[][] sum) {
		for(int i=0; i < 3; i++) {
			if(begin == 0) {
			    if(sum[i][begin] > 0) {
			        return i + 1;
			    }
			    else if(sum[i][begin] < sum[i][end]) {
			        return i + 1;
			    }
			}
			else if(sum[i][begin - 1] < sum[i][end]) {
				return i + 1;
			}
		}
